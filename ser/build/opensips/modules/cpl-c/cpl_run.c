/*
 * $Id: cpl_run.c 5901 2009-07-21 07:45:05Z bogdan_iancu $
 *
...
static inline char *run_reject( struct cpl_interpreter *intr )
{
	...
	/* send the reply */
	if (cpl_fct.sigb.reply(intr->msg, (int)status, &reason, NULL )!=1) {
...
static inline char *run_redirect( struct cpl_interpreter *intr )
{
	...
	/* send the reply */
	if (permanent)
		i = cpl_fct.sigb.reply( intr->msg,301,&cpl_301_reason, NULL);
	else
		i = cpl_fct.sigb.reply( intr->msg,302,&cpl_302_reason, NULL);
