/*
 * $Id: t_fifo.c 6505 2010-01-08 09:51:24Z bogdan_iancu $
 *
...
int parse_tw_append( modparam_t type, void* val)
{
	...
	while (*s) {
		...
		if (elem==0) {
			...
			LM_ERR("short spec '%.*s' unknown(aceepted only body)\n",
					(int)(long)(foo.s-s), s);
				goto error;
			}
			app->add_body = 1;
	...
	return 0;
parse_error:
	LM_ERR("parse error in <%s> around position %ld(%c)\n",
...
int init_twrite_sock(void)
{
	...
	if (flags == -1){
		LM_ERR("init_twrite_sock: fcntl failed: %s\n", strerror(errno));
		close(sock);
		return -1;
	}
